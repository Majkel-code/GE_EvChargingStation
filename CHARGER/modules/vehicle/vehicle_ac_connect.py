...
async def read_items():
    if Vehicle._connected_ac_:
        server_logger.warning(
            f"VEHICLE ACTUALLY CONNECTED: {Vehicle._connected_ac_}"
        )
        ...
        if handshake.ok:
            ...
            if vehicle_specification.ok:
                server_logger.info(
                    f"VEHICLE SUCCESSFULY CONNECTED AND SPECIFICATION READED"
                )
                return {"response": True, "error": None}
            else:
                server_logger.warning(
                    f"VEHICLE SUCCESSFULY CONNECTED BUT SPECIFICATION CAN'T BE TAKEN!"
                )
                return {
                    "response": True,
...
async def read_items():
    if Vehicle._connected_ac_:
        ...
        if disconnect.ok:
            server_logger.info(
                f"VEHICLE SUCCESSFULY DISCONNECTED: {Vehicle._connected_ac_}"
            )
            return {"response": True, "error": None}
    else:
        server_logger.warning(
            f"VEHICLE ACTUALLY IS DISCONNECTED: {Vehicle._connected_ac_}"
        )
        return {
            "response": False,
