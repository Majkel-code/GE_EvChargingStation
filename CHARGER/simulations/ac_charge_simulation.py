...
from CHARGER.config.charger_vehicle_config_bridge import (
    IsServerAlive as Main_server,
)
from CHARGER.config.charger_vehicle_config_bridge import (
    VehicleBridge as Vehicle,
)
from CHARGER.config.logging_system.logging_config import ACChargeSessionLogger
from CHARGER.simulations.charge_simulation import ChargeSimulation
...
class AcVehicle(ChargeSimulation):
    def __init__(self):
        ...
        # VEHICLE INIT
        self.effective_charging_cap = Vehicle.settings_ac[
            "EFFECTIVE_CHARGING_CAP"
        ]
        self.estimated_time_to_full_charge_in_min = None
        self.max_battery_capacity_in_kwh = Vehicle.settings_ac[
    ...
    def check_connectivity_and_set_max_power(self):
        if (
            Vehicle.settings_ac["CHARGING_PORT"]
            in Charger.settings["CHARGING_OUTLETS"]
        ):
            ac_logger.info("CHECKING CONNECTIVITY...")
            time.sleep(2)
            return Charger.settings[
                f"MAX_CHARGING_POWER_{Vehicle.settings_ac['CHARGING_PORT']}"
            ]
        else:
    ...
    def calculate_displayed_time(self, percent, time_in_minutes):
        ...
        ac_logger.info(
            f"Estimated charging time to {percent}% is {h} hours and {m} minutes"
        )

    def estimated_time_needed_to_full_charge(self, percent):
        time_needed = round(
            (self.kw_needed_to_charge_charge / self.max_charging_power), 2
        )
        self.calculate_displayed_time(
            percent=percent, time_in_minutes=time_needed * 60
        )
    ...
    def charged_kw_per_minute(self, charging_after_voltage_drop=None):
        if (
            ...
            kw_per_minute = (
                self.actual_kw_per_min / charging_after_voltage_drop
            )
            Charger.settings["AC_ACTUAL_KW_PER_MIN"] = kw_per_minute
            return kw_per_minute
        else:
            kw_per_minute = (
                self.kw_needed_to_charge_charge
                / self.estimated_time_to_full_charge_in_min
            )
            Charger.settings["AC_ACTUAL_KW_PER_MIN"] = kw_per_minute
            return kw_per_minute
    ...
    def charging_to_max_battery_capacity(self, percent):
        ...
        while (
            ...
            ac_logger.debug(
                f"actual ac battery status: {self.actual_battery_status_in_kwh}"
            )
            ...
            if Vehicle._connected_ac_:
                ...
                self.send_energy()
                ac_logger.info(
                    f"AC CHARGING ONGOING: {self.actual_battery_level}%"
                )
        ...
        if self.actual_battery_level < self.effective_charging_cap:
            ...
            while self.actual_battery_level <= percent:
                ...
                if self.actual_battery_level >= self.effective_charging_cap:
                    ...
                    return {"complete": True, "error": None}
                if Vehicle._connected_ac_:
                    self.actual_battery_level = Vehicle.settings_ac[
                        "BATTERY_LEVEL"
                    ]
                    self.actual_battery_status_in_kwh = Vehicle.settings_ac[
                        "ACTUAL_BATTERY_STATUS_IN_KWH"
                    ...
                    ac_logger.warning(
                        "VEHICLE DISCONNECTED CHARGE SESSION ABORD!"
                    )
                    return {
                        "complete": True,
                        "error": f"Vehicle disconnected from CHARGER! \n"
    ...
    def prepare_ac_charging(self, percent: int = 100):
        ...
        # VEHICLE INIT
        self.percent_to_charge = self.percent_required_to_charge(
            percent=percent
        )
        self.kw_needed_to_charge_charge = self.kw_required_to_charge()
        # CHARGER INIT
        self.estimated_time_to_full_charge_in_min = (
            self.estimated_time_needed_to_full_charge(percent=percent)
        )
        self.actual_kw_per_min = self.charged_kw_per_minute()
        # CHARGE SESSION
        constant_power_level_charging = self.first_stage_charging(
            percent=percent
        )
        if (
            ...
            charging_with_energy_drop = self.charging_to_max_battery_capacity(
                percent=percent
            )
