...
class ChademoVehicle(ChargeSimulation):
    def __init__(self):
        ...
        # VEHICLE INIT
        self.effective_charging_cap = Vehicle.settings_chademo[
    ...
    def check_connectivity_and_set_max_power(self):
        if (
            Vehicle.settings_chademo["CHARGING_PORT"]
    ...
    def charging_to_max_battery_capacity(self, percent):
        ...
        while (
            ...
            Vehicle.take_chademo_vehicle_specification()
            self.actual_battery_level = Vehicle.settings_chademo[
                "BATTERY_LEVEL"
            ]
            self.actual_battery_status_in_kwh = Vehicle.settings_chademo[
                "ACTUAL_BATTERY_STATUS_IN_KWH"
            ...
            chademo_logger.debug(
                "INFORMATION ABOUT ACTUAL CHADEMO CHARGING STATE.... "
            )
            chademo_logger.debug(
                f"chademo actual battery status: {self.actual_battery_status_in_kwh}"
            )
            chademo_logger.debug(
                f"chademo actual kw/min: {self.actual_kw_per_min}"
            )
            chademo_logger.debug(
            ...
            if Vehicle._connected_chademo_:
                ...
                self.send_energy()
                chademo_logger.info(
                    f"CHADEMO CHARGING ONGOING: {self.actual_battery_level}%"
                )
                Charger._energy_is_send_loop_chademo_ += 1
                time.sleep(1)
            else:
                chademo_logger.warning(
                    "VEHICLE DISCONNECTED CHADEMO CHARGE SESSION ABORD!"
                )
                return {
                    "complete": True,
