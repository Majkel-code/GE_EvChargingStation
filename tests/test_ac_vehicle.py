...
class TestAcVehicleEndpoints(TestConfigureServer):
    ...
    def test_check_specific_value(self):
        ...
        for param in self.VEHICLE_EVERY_SETTING:
            ...
            print(f"take_specific_setting test config: {self.test_config['VEHICLE_PROPER_VALUE_AC'][f'{param}']}")
            assert response_take_specific_setting.json() == self.test_config['VEHICLE_PROPER_VALUE_AC'][f'{param}']
