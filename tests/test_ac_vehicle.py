...
class TestAcVehicleEndpoints(TestConfigureServer):
    ...
    def test_check_specific_value(self):
        ...
        for param in self.VEHICLE_EVERY_SETTING:
            response_take_specific_setting = requests.get(
                f'{self.VEHICLE_URL_AC}{param}'
            )
            assert response_take_specific_setting.status_code == 200
            print(
                f"take_specific_setting response json: {response_take_specific_setting.json()}"
            )
            print(
    ...
    def test_negative_read_setting_before_connection(self):
        response_disconnect_vehicle = requests.post(
            f"{self.VEHICLE_URL_AC}{self.test_config['VEHICLE_DISCONNECT_AC']}"
        )
        ...
        assert negative_response_get_vehicle_settings.json() == {
            "detail": f"'{self.test_config['VEHICLE_TAKE_SPECIFIC_KEY_AC']}' CAN'T BE FIND!"
        }
