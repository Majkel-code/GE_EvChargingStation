...
class TestAcVehicleEndpoints(TestConfigureServer):
    ...
    def setUpClass(self) -> None:
        ...
        self.VEHICLE_EVERY_SETTING = self.test_config[
            'VEHICLE_CHECK_EVERY_SETTINGS_AC'
        ]
        self.VEHICLE_URL_CHADEMO = self.test_config['VEHICLE_URL_CHADEMO']
        self.VEHICLE_EVERY_SETTING = self.test_config[
            'VEHICLE_CHECK_EVERY_SETTINGS_CHADEMO'
        ]
        print('vehicle')

    def setUp(self) -> None:
        if not charger_vehicle_config_bridge.VehicleBridge._connected_ac_:
            response_connect = requests.post(
                f"{self.VEHICLE_URL_AC}{self.test_config['VEHICLE_CONNECT_AC']}"
            )
        else:
            pass
    ...
    def test_session_start(self):
        ...
        if (
            self.test_config['START_SESSION_CUSTOM_PERCENT_CHADEMO']
            is not None
            ...
            start_session_url_ac = (
                f"{self.CHARGER_URL}{self.test_config['START_SESSION_AC']}_ac"
            )
    ...
    def test_vehicle_state_saved(self):
        ...
        assert (
            response_check_setting.json()
            > self.test_config['VEHICLE_CHANGE_VALUE_AC']['value']
        )

    def tearDown(self) -> None:
        if charger_vehicle_config_bridge.VehicleBridge._connected_chademo_:
            response_disconnect_vehicle = requests.post(
