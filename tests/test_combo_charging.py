...
class TestAcVehicleEndpoints(TestConfigureServer):
    ...
    def test_reconfigure_parameter(self):
        ...
        # chademo
        response_edit_param_chademo = requests.put(self.VEHICLE_URL_CHADEMO, json=self.test_config['VEHICLE_CHANGE_VALUE_CHADEMO'])
        assert response_edit_param_chademo.status_code == 200
        print(response_edit_param_chademo.json())
        assert response_edit_param_chademo.json() == {"response": True, "error": None}   
        # ac
        response_edit_param_ac = requests.put(self.VEHICLE_URL_AC, json=self.test_config['VEHICLE_CHANGE_VALUE_AC'])
        assert response_edit_param_ac.status_code == 200
        print(response_edit_param_ac.json())
        assert response_edit_param_ac.json() == {"response": True, "error": None}
    ...
    def test_session_start(self):
        ...
        if self.test_config['START_SESSION_CUSTOM_PERCENT_CHADEMO'] is not None and self.test_config['START_SESSION_CUSTOM_PERCENT_AC'] is not None:
            ...
            start_session_url_chademo = f"{self.CHARGER_URL}{self.test_config['START_SESSION_CHADEMO']}_chademo"
        ...
        assert response_session_chademo.json() == {"response": True, "error": None}
